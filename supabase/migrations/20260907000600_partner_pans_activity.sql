-- Add item attribution to the shared order/product timeline. No existing statuses change.
alter table public.wc_order_activity add column order_item_id uuid references public.wc_order_items(id) on delete set null;
create index wc_order_activity_item_idx on public.wc_order_activity(order_item_id);

create or replace function public.wc_set_partner_pans_status(p_item_id uuid,p_source jsonb,p_selection_key text,p_status text)
returns jsonb language plpgsql security definer set search_path=public as $$
declare actor uuid:=auth.uid(); source jsonb; result wc_partner_pans; event jsonb; activity public.wc_order_activity;
begin
 ...
 insert into public.wc_order_activity(order_id,order_item_id,activity_type,message,created_by)
 values((source->>'order_id')::uuid,p_item_id,'note',
  'Partner Pans: '||case when p_status='ordered_and_sent' then 'Ordered and sent' else 'To arrange' end||
  ' — '||coalesce(source->>'product_name','Product')||' · Product quantity: '||coalesce(source->>'quantity','1'),actor::text)
 returning * into activity;
 return to_jsonb(result)||jsonb_build_object('activity',to_jsonb(activity));
end $$;
