...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(body.action==='packages'||body.action==='requote-packages')return json({error:'Packaging is managed only in Products. Reload the report.'},409);
  if(body.action==='calculate-from-products'||body.action==='recalculate-from-products'){
   const requote=body.action==='recalculate-from-products';
   if(requote&&(!['failed','quoted','uncertain'].includes(review.state)||review.token||body.expectedVersion!==review.updated_at))return json({error:'Review changed. Reload before recalculating.'},409);
   if(Deno.env.get('DELIVERY_REVIEW_ENABLED')!=='true')return json({error:'Delivery estimates are not enabled yet.'},503);
   if(!requote&&(review.quote_attempted_at||review.token))return json({ok:true});
   const apiKey=Deno.env.get('FAST_COURIER_API_KEY');
   if(!apiKey)return json({error:'Fast Courier credentials are not configured. The report cannot calculate yet.'},503);
   if(requote||review.state==='approved_without_quote'){
    ...
    if(issue)return json({error:`${issue} Update packaging in Products.`},422);
    const {error}=await db.rpc(requote?'wc_requote_delivery_packages':'wc_save_delivery_packages',{...(requote?{p_review_updated_at:body.expectedVersion}:{}),p_order_id:body.orderId,p_packages:packages,p_signature:reviewSignature(order,rules),p_actor:user.id,p_save_profile:false,p_order_updated_at:order.updated_at,p_items:order.wc_order_items});
    if(error)return json({error:'Review changed. Reload before recalculating.'},409);
   }
