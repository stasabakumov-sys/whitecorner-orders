...
## Обратная связь при загрузке файлов и внешних данных

Любая загрузка файлов или внешних данных в Hub должна показывать состояние выполнения и явный результат. При отказе валидации, ошибке чтения файла, сети, API или сохранения показывайте заметную ошибку рядом с действием: что не удалось, понятную причину и следующий шаг. Для превышения лимита указывайте имя файла, его размер и допустимый предел. Не ограничивайтесь console.log, не скрывайте ошибку автоматически и не показывайте успех до подтверждения сохранения. Сохраняйте введённые данные и ранее сохранённый файл при сбое, позволяйте повторить операцию. Отмена пользователем выбора файла не является ошибкой. Проверяйте сценарии отказа вместе с успешным сценарием.

## Единый дизайн списков Hub

Согласованный пользователем стандарт: каждый фильтр, поиск и управляющая кнопка имеют собственную компактную рамку со скруглением. Не помещайте группу фильтров в дополнительную общую рамку или карточку. Таблица размещается ниже в отдельном белом блоке с тонкой границей и скруглением 12px; между управляющими элементами и таблицей оставляйте 14px. Используйте одинаковую структуру в Products, Orders, Fulfilment и новых или обновляемых списках. Сохраняйте этот вариант до нового указания пользователя.
Заголовки страниц списков оформляйте как Order Costing: h1 без собственного уменьшенного размера или отдельного цвета, margin:0. Счётчики и описание — второстепенным текстом под заголовком.
