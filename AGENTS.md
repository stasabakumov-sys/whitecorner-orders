...
## Стратегическое направление: собственный backend и frontend

Цель владельца проекта — постепенно перенести данные и необходимые бизнес-функции Wix в собственный backend White Corner, создать свой frontend и затем соединить их. Учитывайте это во всех задачах: развивайте независимые модели и серверные контракты, оставляя интеграцию с Wix отдельным адаптером. Это направление развития, а не разрешение на неподтверждённое отключение Wix или расширение каждой задачи до полной миграции.

- На переходном этапе явно определяйте источник истины для каждого поля. Wix остаётся источником импортируемых данных до отдельного решения о переключении; локальные операционные поля принадлежат Hub. Не вводите бесконтрольную двустороннюю запись.
- Используйте существующие внутренние ID и сохраняйте связи с упаковкой, чертежами, себестоимостью и заказами. Сопоставляйте импорт по внешним ID; совпадение названия — кандидат для сверки, а не достаточное основание для объединения товаров.
- Импорт должен быть повторяемым, возобновляемым и проверяемым по сохранённым данным. Частичная загрузка или ошибка доступа не означают удаление отсутствующих записей.
- Сохраняйте полную семантику вариантов: цвет может влиять на цену, SKU, остатки и производство. Исключение цвета из подбора упаковки не распространяется на каталог и расчёт себестоимости.
- Не переписывайте исторические позиции заказов и финансовые снимки текущими данными каталога. Не включайте возвраты, платежи, документы или запись в Wix до реализации и авторизации соответствующей интеграции.
- Будущий публичный frontend должен получать отдельное публичное представление каталога через безопасный контракт. Операционные таблицы Hub, customer data, secrets и приватные чертежи не становятся публичными из-за появления витрины.

Исходный аудит и план переноса каталога: [docs/WIX_CATALOG_MIGRATION.md](docs/WIX_CATALOG_MIGRATION.md). Проверяйте актуальное состояние перед реализацией; числа в аудите — снимок на указанную дату.

Согласованный владельцем порядок: **этап 1 — отдельный frontend на Next.js App Router + TypeScript с Wix как backend для каталога, корзины, checkout и заказов; параллельно развивается Angular/PrimeNG Hub. Этап 2 — соединить frontend и собственный backend Hub через API, постепенно перенести соответствующие функции с Wix.** Импорт каталога в Hub не является предварительным условием запуска frontend. На первом этапе обращения к Wix изолируются адаптером витрины, без зависимости от готовности API Hub; секретные обращения остаются серверными. На втором этапе оба интерфейса работают через собственные серверные контракты, а не вызывают UI друг друга. Это план, не указание немедленно создавать новые репозитории или менять hosting. Уточнения и ограничения описаны в плане переноса каталога.
...
## Иконки действий в интерфейсе Hub

Согласованный пользователем стандарт: действия **Add**, **Edit** и **Save** в новых или затрагиваемых интерфейсах оформляйте компактными квадратными кнопками-иконками по образцу CNC. Иконка должна быть строго отцентрирована по горизонтали и вертикали; у кнопки обязательны понятные `aria-label` и tooltip/title. Не заменяйте такими иконками все старые кнопки отдельной массовой правкой: применяйте стандарт, когда соответствующий интерфейс создаётся или меняется.

## Второй уровень меню Hub

Все раскрывающиеся разделы бокового меню оформляйте одинаково по образцу **Packing**: одинаковые размер шрифта, высота строк, отступы, скругление и выделение активного пункта. Используйте общие стили второго уровня для Packing, Reference и будущих разделов; не задавайте отдельный стиль подпунктам одного раздела без нового указания пользователя. Проверяйте вид в браузере на широком и узком экране.
