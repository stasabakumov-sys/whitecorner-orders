...
function setup(quantity=1,options?:any){
 const saved:any={};
 const from=vi.fn((table:string)=>{let rows:any;const q:any={delete:()=>q,eq:()=>q,update:()=>q,insert:(r:any)=>{rows=r;saved[table]=structuredClone(r);return q;},select:()=>q,order:()=>q,range:async()=>({data:[],error:null}),maybeSingle:async()=>({data:saved[table]||null,error:null}),then:(resolve:any)=>resolve({data:rows||saved[table]||[],error:null})};return q;});
