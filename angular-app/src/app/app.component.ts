...
@Component({
  ...
  template: `
    @if (!auth.ready()) {
      ...
      <main>
        <div class="content" [class.email-content]="router.url.startsWith('/email')" [class.finance-content]="router.url.startsWith('/finance')" [class.production-content]="router.url.startsWith('/production')">
          @if (router.url.startsWith('/email')) {
            <div class="gmail-connect-bar">
              ...
              <div class="gmail-account" [class.connected]="email.isConnected('info')">
                ...
                @if (email.isConnected('info')) {<strong>Connected</strong><button class="reconnect" type="button" (click)="email.connect('info')" [disabled]="email.loading()">Reconnect</button>} @else {<button type="button" (click)="email.connect('info')" [disabled]="email.loading()">Connect</button>}
              </div>
              <div class="gmail-account" [class.connected]="email.isConnected('support')">
                <span>support@whitecorner.com.au</span>
                @if (email.isConnected('support')) {<strong>Connected</strong><button class="reconnect" type="button" (click)="email.connect('support')" [disabled]="email.loading()">Reconnect</button>} @else {<button type="button" (click)="email.connect('support')" [disabled]="email.loading()">Connect</button>}
              </div>
  ...
  styles: [`@layer hub-layout {
    ...
    .content.production-content{overflow:hidden;padding-bottom:0}
