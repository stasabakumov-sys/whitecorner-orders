...
@Component({
  ...
  template: `
    <section class="page-card">
      ...
      <div class="list-table"><p-table [value]="visible()" [tableStyle]="{ 'min-width': '54rem' }" [rowHover]="true">
...
export class FulfilmentComponent implements OnInit {
  ...
  getQuotes(row:FulfilmentRow,ps:string,pstate:string,pp:string,ptype:string,ptail:boolean,ds:string,dstate:string,dp:string,dtype:string,dtail:boolean){
    ...
    const request:FastCourierQuoteRequest={
      ...
      items:this.f.packagesFor(shipment.id).map(p=>({type:'box',weight:Number(p.weight_kg),length:Number(p.length_mm)/10,width:Number(p.width_mm)/10,height:Number(p.height_mm)/10,quantity:1,contents:'General'})),
    };
