...
@Component({selector:'app-customers',standalone:true,imports:[DatePipe,FormsModule,DialogModule],template:`
  <header><div><h1>Customers</h1><p>Saved Wix contacts, including people without orders. @if(customers.syncedAt()){Updated {{customers.syncedAt()|date:'MMM d, yyyy, h:mm a':'+1000'}} AEST.}</p></div><button (click)="customers.load(true)" [disabled]="customers.loading()">{{customers.syncing()?'Updating…':'Refresh Wix'}}</button></header>
  @if(customers.error()){<p class="notice error" role="alert">{{customers.error()}}</p>}
  @if(customers.loading()){<p class="notice" role="status">{{customers.syncing()?'Updating saved contacts from Wix…':'Loading saved contacts…'}}</p>}
  <div class="filters">
