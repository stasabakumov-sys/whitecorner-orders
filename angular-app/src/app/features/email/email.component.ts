    ...
    @if(section()==='Mail'){
      <div class="mail-app">
        ...
        <section class="message-list">
          ...
          @if(mailError()){<div class="mail-error"><i class="pi pi-exclamation-circle"></i><span>{{mailError()}}</span><button (click)="mailError.set('')">×</button></div>}
          <div class="mail-list-scroll">
            @if(!mailReady()){<div class="mail-loading"><i class="pi pi-spin pi-spinner"></i><span>Loading mail…</span></div>} @else {
              ...
              @if(canLoadMore()){<button class="load-more-mail" (click)="loadMoreMail()" [disabled]="loadingOlder()">@if(loadingOlder()){<i class="pi pi-spin pi-spinner"></i> Loading older messages…} @else {Load older messages}</button>}
            }
          </div>
        </section>
      ...
      @if(selected();as mail){
        <section class="mail-reader">
          <div class="reader-toolbar">
            ...
            @if(mail.status==='Archive'){<button class="reader-icon" title="Move to inbox" aria-label="Move to inbox" (click)="modifyMessage(mail,'moveToInbox')" [disabled]="messageActionBusy()"><i class="pi pi-inbox"></i></button>}
            @if(mail.status==='Trash'){<button class="reader-icon" title="Restore from trash" aria-label="Restore from trash" (click)="modifyMessage(mail,'untrash')" [disabled]="messageActionBusy()"><i class="pi pi-undo"></i></button>}
            ...
            <button class="reader-icon" title="Reply" aria-label="Reply" (click)="startReply(replyTarget(mail))" [disabled]="messageActionBusy()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 17-6-5 6-5v3c7 0 10 3 12 7-3-2-6-3-12-2z"/></svg></button>
            <button class="reader-icon" title="Forward" aria-label="Forward" (click)="startForward(replyTarget(mail))" [disabled]="messageActionBusy()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 17 6-5-6-5v3C8 10 5 13 3 17c3-2 6-3 12-2z"/></svg></button>
          </div>
          <div class="reader-scroll">
            <div class="reader-content">
              @if(mailError()){<div class="reader-error"><i class="pi pi-exclamation-circle"></i><span>{{mailError()}}</span><button (click)="mailError.set('')">×</button></div>}
              <div class="reader-subject"><h2>{{mail.subject}}</h2><span class="folder-label">{{mail.status}}</span>@if(mail.needs_reply){<p-tag value="Needs reply" severity="warn"/>}</div>
              @if(bodyLoading()===mail.mailbox+':'+mail.id&&!threadMessages().length){<div class="reader-body thread-loading"><i class="pi pi-spin pi-spinner"></i> Loading conversation…</div>}
              <div class="message-thread-list">
                @for(message of threadMessages();track message.id;let last=$last){
                  <article class="thread-message" [class.collapsed]="!threadExpanded(message.id)">
                    <button class="thread-message-head" type="button" (click)="toggleThreadMessage(message)">
                      <span class="reader-avatar" [style.background-color]="avatarColor(message.email)" aria-hidden="true">{{message.initials}}</span>
                      <span class="thread-sender"><b>{{message.correspondent}}</b><small>{{message.email}}</small></span>
                      @if(!threadExpanded(message.id)){<span class="thread-snippet">{{message.preview||message.body}}</span>}
                      <time>{{message.time}}</time><i [class]="threadExpanded(message.id)?'pi pi-chevron-up':'pi pi-chevron-down'"></i>
                    </button>
                    @if(threadExpanded(message.id)){
              ...
              @if(replyMode()==='none'){
                <div class="gmail-actions">
                  ...
                  <button (click)="startReplyAll(replyTarget(mail))"><i class="pi pi-reply"></i><span>Reply all</span></button>
                  <button (click)="startForward(replyTarget(mail))"><i class="pi pi-forward"></i><span>Forward</span></button>
                </div>
              } @else {
                <div class="reader-reply active">
                  <div class="reply-label"><i [class]="replyMode()==='forward'?'pi pi-forward':'pi pi-reply'"></i><b>{{replyMode()==='forward'?'Forward':replyMode()==='replyAll'?'Reply all':'Reply'}}</b><span>from {{mailboxAddress(mail.mailbox)}}</span><button class="close-compose" (click)="cancelReply()">×</button></div>
                  ...
                  <div class="conversation-included"><i [class]="replyMode()==='forward'?'pi pi-forward':'pi pi-comments'"></i><div><b>{{replyMode()==='forward'?'Original message will be forwarded':'Original conversation will be included'}}</b><span>{{replyMode()==='forward'?(mail.attachments?.length||0)+' attachment(s) will also be forwarded':replyMode()==='replyAll'?'The sender and original recipients will receive this reply':'The reply stays in the existing Gmail thread'}}</span></div></div>
...
:host{display:block;height:100%;min-height:0;overflow:hidden}  .email-page{min-width:0;height:100%;min-height:0;display:flex;flex-direction:column;overflow:hidden}.page-head{display:flex;align-items:center;gap:10px;margin:0 0 5px;flex:0 0 32px}.page-head>div:first-child{min-width:0}.page-head h2{margin:0;font-size:18px;font-weight:600;color:#101828;line-height:1}.page-head p{display:none}.top-actions{margin-left:auto;display:flex;align-items:center;gap:6px}.top-actions ::ng-deep .p-button{padding:.35rem .65rem;font-size:11px}.top-search{position:relative;width:min(360px,32vw)}.top-search i{position:absolute;left:11px;top:50%;transform:translateY(-50%);font-size:11px;color:#98a2b3;pointer-events:none}.top-search input{width:100%;height:32px;padding-left:31px;border-radius:18px;font-size:11px;background:#f7f8fa}
  ...
  .mail-reader{position:fixed;z-index:1200;top:0;left:205px;right:0;bottom:0;background:#fff;display:flex;flex-direction:column}.reader-toolbar{height:42px;flex:0 0 42px;border-bottom:1px solid #e4e7ec;display:flex;align-items:center;padding:0 14px;gap:3px;background:#fff}.back-button,.reader-icon{width:28px;height:28px;border:0;background:transparent;border-radius:50%;display:grid;place-items:center;color:#475467;cursor:pointer}.reader-icon svg{width:16px;height:16px}.back-button:hover:not(:disabled),.reader-icon:hover:not(:disabled){background:#f2f4f7}.back-button:disabled,.reader-icon:disabled{opacity:.42;cursor:default}.back-button i,.reader-icon i{font-size:14px}.reader-icon.star-action.active{color:#d39a00}.reader-icon.danger-action:hover:not(:disabled){background:#fff1f1;color:#b42318}.reader-action-status{font-size:9px;color:#98a2b3;margin-left:3px;white-space:nowrap}.reader-action-status.error{color:#b42318}.reader-toolbar-spacer{flex:1}.reader-toolbar-divider{width:1px;height:18px;background:#e4e7ec;margin:0 2px}.reader-scroll{overflow:auto;min-height:0;flex:1}.reader-content{max-width:1080px;margin:0 auto;padding:20px 32px 42px}.reader-subject{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.reader-subject h2{font-size:19px;line-height:1.3;font-weight:500;color:#202124;margin:0}.folder-label{display:inline-flex;align-items:center;height:18px;padding:0 7px;border-radius:3px;background:#eef0f2;color:#5f6368;font-size:9px;font-weight:500;line-height:1;white-space:nowrap}.reader-avatar{width:30px;height:30px;border-radius:50%;display:grid;place-items:center;color:#fff;font-size:10px;font-weight:500;letter-spacing:.01em;line-height:1;flex:0 0 30px}.reader-body{white-space:pre-wrap;overflow-wrap:anywhere;font-size:12px;line-height:1.5;color:#202124;margin:0;min-height:24px;max-width:100%}.thread-loading{display:flex;align-items:center;gap:7px;margin-top:18px;color:#98a2b3}.message-thread-list{display:grid;gap:9px;margin-top:17px}.thread-message{min-width:0;border:1px solid #dfe3e8;border-radius:10px;background:#fff;overflow:hidden}.thread-message.collapsed{background:#fbfcfd}.thread-message-head{width:100%;min-width:0;display:grid;grid-template-columns:30px minmax(110px,180px) minmax(0,1fr) auto 14px;align-items:center;gap:10px;border:0;background:transparent;padding:10px 12px;text-align:left;color:#344054;cursor:pointer}.thread-message-head:hover{background:#f8fafc}.thread-sender{display:flex;min-width:0;flex-direction:column;justify-content:center;gap:2px;min-height:30px}.thread-sender b,.thread-sender small,.thread-snippet{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.thread-sender b{font-size:11px;color:#202124}.thread-sender small{font-size:9px;color:#667085}.thread-snippet{font-size:10px;color:#7b8798}.thread-message-head time{font-size:9px;color:#98a2b3;white-space:nowrap}.thread-message-head>i{font-size:9px;color:#98a2b3}.thread-message-content{min-width:0;padding:2px 52px 17px}.reader-ai{margin-top:22px;padding:10px 12px;border:1px solid #e4e7ec;border-radius:9px;background:#fbfcfd;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.reader-ai>div{display:flex;flex-direction:column;gap:3px;min-width:0}.reader-ai small{text-transform:uppercase;font-size:8px;color:#98a2b3;font-weight:700}.reader-ai b{font-size:10px;color:#475467}.reader-reply{margin-top:18px;border:1px solid #dfe3e8;border-radius:9px;overflow:hidden}.reader-reply textarea{display:block;width:100%;min-height:120px;border:0;resize:vertical;padding:12px;font:inherit;font-size:11px;outline:none;box-sizing:border-box}.reader-reply .reply-actions{padding:8px 10px}@media(max-width:880px){.mail-reader{left:0;top:0}.reader-content{padding:18px 16px}.reader-avatar{width:28px;height:28px;flex-basis:28px;font-size:9px}.thread-message-head{grid-template-columns:28px minmax(90px,140px) minmax(0,1fr) auto}.thread-message-head>i{display:none}.thread-message-content{padding:2px 14px 15px}.reader-ai{grid-template-columns:1fr 1fr}}
...
export class EmailComponent implements OnDestroy{
  readonly section=signal<EmailTab>('Mail');readonly views:MailView[]=['Inbox','Unread','Needs reply','Starred','Sent','Archive','Trash'];readonly activeView=signal<MailView>('Inbox');readonly activeMailbox=signal<MailboxId>('all');readonly query=signal('');
  readonly rows=signal<MailRow[]>([]);
  readonly selected=signal<MailRow|null>(null);
  readonly threadMessages=signal<ThreadMail[]>([]);
  readonly expandedThreadMessages=signal<Set<string>>(new Set());
  ...
  readonly replyMode=signal<'none'|'reply'|'replyAll'|'forward'>('none');
  ...
  private viewKey(mailbox:'info'|'support',view:GmailView){return`${mailbox}:${view}`;}
  ...
  private hydrateMailCache(){
    ...
    for(const mailbox of ['info','support'] as const){
      for(const view of ['Inbox','Sent','Archive','Trash'] as const){
  ...
  private fetchView(mailbox:'info'|'support',view:GmailView,refresh=false):Promise<void>{
  ...
  canLoadMore(){const box=this.activeMailbox();const keys:('info'|'support')[]=box==='all'?['info','support']:[box];return keys.some(key=>this.sourceViews(this.activeView()).some(view=>this.email.hasMore(key,view)));}
  async loadMoreMail(){
    if(this.loadingOlder())return;this.loadingOlder.set(true);this.mailError.set('');
    const box=this.activeMailbox();const keys:('info'|'support')[]=box==='all'?['info','support']:[box];const sources=this.sourceViews(this.activeView());
    try{await Promise.all(keys.flatMap(mailbox=>sources.filter(view=>this.email.hasMore(mailbox,view)).map(async view=>this.absorbBatch(mailbox,view,await this.email.loadMore(mailbox,view)))));}
    catch(e){this.mailError.set(String((e as Error)?.message||e));}
    ...
    try{
      if(mail.threadId){
        ...
        if(messages.length){
          ...
          const clicked=messages.find(message=>message.id===mail.id)||messages[messages.length-1];
          current={...current,body:clicked.body,html_body:clicked.html_body,attachments:clicked.attachments,images_blocked:clicked.images_blocked,threadId:thread.id};
        }
  ...
  avatarColor(email:string){const palette=['#526DA5','#8A5D3B','#397A69','#765AA8','#A14F68','#36729A','#6E783A','#8A5648','#496F78','#7D587F'];let hash=0;for(const char of email.trim().toLowerCase())hash=((hash*31)+char.charCodeAt(0))>>>0;return palette[hash%palette.length];}
  ...
  private async loadMessageMedia(mail:MailRow,version:number){
    const key=`${mail.mailbox}:${mail.id}`;this.mediaLoading.set(key);
    try{
      const full=await this.email.getMessage(mail.mailbox,mail.id,true);
      if(version!==this.selectionVersion||this.selected()?.id!==mail.id||this.selected()?.mailbox!==mail.mailbox)return;
      const current=this.mergeMessageDetail(mail,full);
      this.rows.update(rows=>rows.map(row=>row.id===mail.id&&row.mailbox===mail.mailbox?current:row));this.selected.set(current);
      void this.loadImagePreviews(current,version);
  ...
  async modifyMessage(mail:MailRow,action:'markRead'|'markUnread'|'archive'|'moveToInbox'|'trash'|'untrash'|'star'|'unstar'){
    ...
    try{
      ...
      if(action==='archive'||action==='moveToInbox'||action==='trash'||action==='untrash'){
        const destination:GmailView=action==='archive'?'Archive':action==='trash'?'Trash':'Inbox';
        this.loadedViews.delete(this.viewKey(mail.mailbox,destination));this.email.invalidateList(mail.mailbox,destination);
  ...
  async displayExternalImages(mail:MailRow){
    ...
    try{
      ...
      if(version===this.selectionVersion)void this.loadImagePreviews({...mail,...patch},version);
    }catch(e){if(version===this.selectionVersion)this.mailError.set(String((e as Error)?.message||e));}
  ...
  private async loadImagePreviews(mail:MailRow,version:number){
    await Promise.all(this.imageAttachments(mail).map(async attachment=>{
  ...
  private clearAttachmentPreviews(){for(const url of Object.values(this.attachmentPreviews()))URL.revokeObjectURL(url);this.attachmentPreviews.set({});}
  ngOnDestroy(){this.clearAttachmentPreviews();}
  async downloadAttachment(mail:MailRow,attachment:GmailAttachment){
    ...
    try{const blob=await this.email.downloadAttachment(mail.mailbox,mail.id,attachment);const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=attachment.filename||'attachment';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);this.messageActionStatus.set('');}
  ...
  startReplyAll(mail:MailRow){
    this.replyMode.set('replyAll');this.forwardTo.set('');this.sendStatus.set('');this.replyText.set(mail.draft_reply||'');
  }
  startForward(mail:MailRow){
    this.replyMode.set('forward');this.forwardTo.set('');this.sendStatus.set('');this.replyText.set('');
  }
  cancelReply(){this.replyMode.set('none');this.replyText.set('');this.forwardTo.set('');this.sendStatus.set('');this.sending.set(false);}
  canSend(mail:MailRow){if(this.replyMode()==='forward')return this.forwardTo().trim().includes('@');return!!this.replyText().trim()&&!!this.replyTarget(mail).email;}
  async sendCurrent(mail:MailRow){
    if(!this.canSend(mail)||this.sending())return;
    mail=this.replyTarget(mail);
    ...
    try{
      ...
      await this.email.send(mail.mailbox,to,subject,this.replyText(),[],{mode:forward?'forward':this.replyMode()==='replyAll'?'replyAll':'reply',sourceMessageId:mail.id,threadId:mail.threadId});
      if(mail.threadId&&!forward){
        try{const thread=await this.email.getThread(mail.mailbox,mail.threadId,true);const root=this.selected()||mail;const messages=thread.messages.map(message=>this.toThreadMail(root,message));this.threadMessages.set(messages);this.expandedThreadMessages.set(new Set([messages.at(-1)?.id||mail.id]));}
        catch(e){console.warn('Thread refresh after send failed',e);}
      }
