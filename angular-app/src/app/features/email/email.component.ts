...
@Component({
  ...
  <section class="email-page">
    <div class="page-head">
      ...
      <div class="top-actions"><p-button label="Mail" icon="pi pi-envelope" [outlined]="section()!=='Mail'" (onClick)="section.set('Mail')"/><p-button label="AI Agent" icon="pi pi-sparkles" [outlined]="section()!=='AI Agent'" (onClick)="section.set('AI Agent')"/></div>
    </div>
    ...
    @if(section()==='Mail'){
      <div class="mail-app">
        <aside class="mail-nav">
          ...
          <button class="mailbox-option" [class.active]="activeMailbox()==='all'" (click)="setMailbox('all')"><span class="mailbox-dot all"></span><span>All mail</span><small>{{mailboxCount('all')}}</small></button>
          <button class="mailbox-option" [class.active]="activeMailbox()==='info'" (click)="setMailbox('info')"><span class="mailbox-dot info"></span><span>info@whitecorner.com.au</span><small>{{mailboxCount('info')}}</small></button>
          <button class="mailbox-option" [class.active]="activeMailbox()==='support'" (click)="setMailbox('support')"><span class="mailbox-dot support"></span><span>support@whitecorner.com.au</span><small>{{mailboxCount('support')}}</small></button>
          <div class="nav-label folders-label">Folders</div>
          @for(view of views;track view){<button class="folder-option" [class.active]="activeView()===view" (click)="setView(view)"><i [class]="folderIcon(view)"></i><span>{{view}}</span><small>{{countFor(view)}}</small></button>}
          <div class="mail-status live"><span></span><div><b>Gmail connected</b><small>Live Inbox / Sent</small></div></div>
        </aside>

        <section class="message-list">
          <div class="list-toolbar"><div class="list-title"><b>{{activeView()}}</b><span>{{activeMailboxLabel()}} · {{visibleRows().length}} messages</span></div><div class="search"><i class="pi pi-search"></i><input pInputText placeholder="Search mail" [value]="query()" (input)="query.set($any($event.target).value);ensureSelection()"/></div></div>
      ...
      @if(selected();as mail){
        <section class="mail-reader">
          <div class="reader-toolbar">
            ...
            <button class="reader-icon" title="Reply"><i class="pi pi-reply"></i></button>
            <button class="reader-icon" title="More"><i class="pi pi-ellipsis-v"></i></button>
          </div>
          <div class="reader-scroll">
            <div class="reader-content">
              <div class="reader-subject"><h2>{{mail.subject}}</h2>@if(mail.needs_reply){<p-tag value="Needs reply" severity="warn"/>}</div>
              <div class="reader-mailbox">{{mailboxAddress(mail.mailbox)}} · {{mail.received_at}}</div>
              ...
              <div class="reader-reply">
                ...
                <textarea [value]="mail.draft_reply||''" placeholder="Reply…" disabled></textarea>
                <div class="reply-actions"><span>Reply/send will be enabled after live mail verification.</span><p-button label="Send" icon="pi pi-send" [disabled]="true"/></div>
              </div>
            </div>
          </div>
        </section>
...
export class EmailComponent{
  ...
  closeMail(){this.selected.set(null);}
  ...
  mailboxCount(box:MailboxId){return this.rows().filter(row=>(box==='all'||row.mailbox===box)&&row.status==='Inbox').length;}
