            ...
            <div class="reader-content">
              ...
              @if(replyMode()==='none'){
                ...
                <div class="reader-reply active">
                  ...
                  <div class="conversation-included"><i [class]="replyMode()==='forward'?'pi pi-forward':'pi pi-comments'"></i><div><b>{{replyMode()==='forward'?'Original message will be forwarded':'Original conversation will be included'}}</b><span>{{replyMode()==='forward'?(mail.attachments?.length||0)+' attachment(s) will also be forwarded':'The reply stays in the existing Gmail thread'}}</span></div></div>
...
:host{display:block;height:100%;min-height:0;overflow:hidden}  .email-page{min-width:0;height:100%;min-height:0;display:flex;flex-direction:column;overflow:hidden}.page-head{display:flex;align-items:center;gap:10px;margin:0 0 5px;flex:0 0 32px}.page-head>div:first-child{min-width:0}.page-head h2{margin:0;font-size:18px;font-weight:600;color:#101828;line-height:1}.page-head p{display:none}.top-actions{margin-left:auto;display:flex;align-items:center;gap:6px}.top-actions ::ng-deep .p-button{padding:.35rem .65rem;font-size:11px}.top-search{position:relative;width:min(360px,32vw)}.top-search i{position:absolute;left:11px;top:50%;transform:translateY(-50%);font-size:11px;color:#98a2b3;pointer-events:none}.top-search input{width:100%;height:32px;padding-left:31px;border-radius:18px;font-size:11px;background:#f7f8fa}
  ...
  .conversation-included{display:flex;align-items:center;gap:9px;margin:0 12px 10px;padding:8px 10px;border:1px solid #e4e7ec;border-radius:8px;background:#f8fafc;color:#667085}.conversation-included>i{font-size:12px}.conversation-included>div{display:flex;flex-direction:column;gap:2px}.conversation-included b{font-size:10px;color:#475467}.conversation-included span{font-size:9px}
...
export class EmailComponent implements OnDestroy{
  ...
  closeMail(){
    ...
    this.selectionVersion++;this.bodyLoading.set('');this.readerHistoryPushed=false;this.cancelReply();this.clearAttachmentPreviews();this.selected.set(null);
  ...
  private async showMail(mail:MailRow){
    ...
    let current=mail;
    try{
      if(!mail.body||mail.images_blocked){const full=await this.email.getMessage(mail.mailbox,mail.id,true);current={...mail,body:this.normalizeMessageBody(full.body,mail.preview),html_body:full.html||'',images_blocked:full.imagesBlocked===true,attachments:full.attachments||[],received_at:full.date||mail.received_at,time:this.formatMailTime(full.date||mail.received_at),unread:full.unread??mail.unread,starred:full.starred??mail.starred};this.rows.update(rows=>rows.map(row=>row.id===mail.id&&row.mailbox===mail.mailbox?current:row));}
  ...
  private async loadImagePreviews(mail:MailRow,version:number){
    for(const attachment of this.imageAttachments(mail)){
      const key=this.attachmentKey(mail,attachment);if(this.attachmentPreviews()[key])continue;
      try{
  ...
  async sendCurrent(mail:MailRow){
    ...
    try{
      ...
      await this.email.send(mail.mailbox,to,subject,this.replyText(),[],{mode:forward?'forward':'reply',sourceMessageId:mail.id,threadId:mail.threadId});
