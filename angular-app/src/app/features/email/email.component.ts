...
import { EmailService, GmailAttachment, GmailMessageDetail, GmailMessageRow, GmailView } from '../../core/services/email.service';
        ...
        <section class="mail-reader">
          ...
          <div class="reader-scroll">
            <div class="reader-content">
              ...
              @if(mediaLoading()===mail.mailbox+':'+mail.id){<div class="email-media-loading"><i class="pi pi-spin pi-spinner"></i><span>Loading images…</span></div>}
              @if(mail.images_blocked){<div class="email-media-notice"><span>Some images could not be loaded.</span><button (click)="displayExternalImages(mail)" [disabled]="bodyLoading()!==''">Try again</button></div>}
              @if(imageAttachments(mail).length){<div class="email-image-previews">@for(attachment of imageAttachments(mail);track attachment.attachmentId){@if(attachmentPreview(mail,attachment);as preview){<button type="button" (click)="downloadAttachment(mail,attachment)" [title]="'Download '+attachment.filename"><img [src]="preview" [alt]="attachment.filename"><span>{{attachment.filename}}</span></button>}@else{<div class="image-preview-loading">Loading image…</div>}}</div>}
              ...
              @if(replyMode()==='none'){
                <div class="gmail-actions">
                  ...
                  <button (click)="startReplyAll(mail)"><i class="pi pi-reply"></i><span>Reply all</span></button>
...
export class EmailComponent implements OnDestroy{
  ...
  closeMail(){
    ...
    this.selectionVersion++;this.bodyLoading.set('');this.mediaLoading.set('');this.readerHistoryPushed=false;this.cancelReply();this.clearAttachmentPreviews();this.selected.set(null);
  ...
  private async showMail(mail:MailRow){
    ...
    this.bodyLoading.set(current.body?'':loadingKey);this.mediaLoading.set('');this.selected.set(current);
    try{
      if(!current.body){const full=await this.email.getMessage(mail.mailbox,mail.id,false);current=this.mergeMessageDetail(mail,full);this.rows.update(rows=>rows.map(row=>row.id===mail.id&&row.mailbox===mail.mailbox?current:row));}
      ...
      if(current.images_blocked)void this.loadMessageMedia(current,version);
  ...
  canSend(mail:MailRow){if(this.replyMode()==='forward')return this.forwardTo().trim().includes('@');return!!this.replyText().trim()&&!!mail.email;}
