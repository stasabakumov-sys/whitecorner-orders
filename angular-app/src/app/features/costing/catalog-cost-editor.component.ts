...
@if(showWork){<h4>Work cost for one unit · incl. GST</h4><div class="fields">@for(w of categories;track w.key){<label>{{w.label}}<input type="number" min="0" step="0.01" [(ngModel)]="work[w.key]" placeholder="Unknown"></label>}</div>}
...
<p>@if(showWork){Blank = unknown. Zero = no cost. }Saved order calculations stay unchanged.</p>
...
export class CatalogCostEditorComponent implements OnChanges {
 ...
 options(){return Object.entries(this.part.options||{}).filter(([key])=>!this.hideColour||!['colour','color'].includes(key.toLowerCase())).map(([k,v])=>`${k}: ${v}`).join(' · ')||'No structural options';}
 ...
 async save(){if(this.invalid())return;if(this.part.backdrop_material_scope){if(!await this.s.saveBackdropMaterialProfile(this.part,this.lines,this.confirmed,this.part.profile?.updated_at??null))return;}else if(this.part.cart_material_scope){if(!await this.s.saveCartMaterialProfile(this.part,this.lines,this.confirmed,this.part.profile?.updated_at??null))return;}else{const targets=this.part.shared_parts?.length?this.part.shared_parts:[this.part];for(const target of targets){const work=this.showWork?this.work:(target.profile?.work_costs||this.work);if(!await this.s.saveCatalogProfile(target,this.lines,work,this.pans,this.confirmed,target.profile?.updated_at??null))return;}}this.version=this.s.profiles().find(p=>p.variant_key===this.part.variant_key)?.updated_at??null;this.saved=true;}
}
