...
export class CatalogCostEditorComponent implements OnChanges {
 ...
 async save(){if(this.invalid())return;const targets=this.part.shared_parts?.length?this.part.shared_parts:[this.part];for(const target of targets){const work=this.showWork?this.work:(target.profile?.work_costs||this.work);if(!await this.s.saveCatalogProfile(target,this.lines,work,this.pans,this.confirmed,target.profile?.updated_at??null))return;}this.version=this.s.profiles().find(p=>p.variant_key===this.part.variant_key)?.updated_at??null;this.saved=true;}
}
