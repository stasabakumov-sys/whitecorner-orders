...
describe('Shared Painting cost for every product',()=>{
 ...
 it('excludes unordered add-on minutes from the Board configuration',()=>{const component=new ProductWorkCostComponent({} as any);component.product={id:'main',product_name:'Display Unit'};component.orderVariant=true;component.orderComponentIds=['main'];component.templates=[{id:'template',name:'Display Unit',parts:[{id:'body',component_product_id:'main',name:'Body'},{id:'back',component_product_id:'addon',name:'Back panel'}],estimates:{CNC:10,'Assembly:body':20,'Assembly:back':60},version:1}];expect(component.displayTemplates()[0].parts.map(part=>part.id)).toEqual(['body']);expect(component.displayTemplates()[0].estimates['Assembly:back']).toBeUndefined();});
});
