...
export class ProductWorkCostComponent implements OnChanges{
 ...
 async load(){const id=this.product?.id,token=++this.token;this.templates=[];this.rates=[];this.error='';this.loading=false;if(!id||this.product.saved_only)return;this.loading=true;
  const [templates,rates]=await Promise.all([this.db.client.from('wc_shop_templates').select('*').eq('product_id',id).order('name'),this.db.client.from('wc_work_rates').select('*').order('sort_order')]);
  if(token!==this.token)return;this.loading=false;if(templates.error||rates.error){this.error='Could not calculate planned work cost. Refresh and retry.';return;}this.templates=(templates.data||[]) as ShopTemplate[];this.rates=(rates.data||[]) as WorkRate[];
 }
 rows(template:ShopTemplate):PlannedWorkRow[]{return plannedWorkRows(template,this.rates);}total(template:ShopTemplate,painted:boolean){return plannedTotal(this.rows(template),painted);}
}
