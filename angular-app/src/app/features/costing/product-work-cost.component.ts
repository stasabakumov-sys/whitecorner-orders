...
export class ProductWorkCostComponent implements OnChanges{
 ...
 @Input() product:any;@Input() sizes:string[]=[];@Input() selectedSize='';@Input() selectedFolding:Folding|''='';@Input() manualSizes=false;@Input() materialProfiles:any[]=[];@Input() materials:any[]=[];templates:ShopTemplate[]=[];rates:WorkRate[]=[];loading=false;error='';private token=0;
 ...
 displayTemplates(){return this.hasFolding?(this.selectedFolding?[this.selectedFolding]:['foldable','nonfoldable'] as Folding[]).flatMap(fold=>{const shared=this.templates.find(t=>!t.size_key&&t.folding===fold);if(shared)return[shared];const legacy=this.templates.filter(t=>t.folding===fold);return legacy.length===1?[legacy[0]]:[];}):this.templates;}
