...
export class CostingService {
 materials=signal<any[]>([]);groups=signal<any[]>([]);prices=signal<any[]>([]);profiles=signal<any[]>([]);costs=signal<any[]>([]);orders=signal<any[]>([]);parts=signal<any[]>([]);pans=signal<any[]>([]);
 ...
 async load(){if(this.loading())return;this.loading.set(true);this.error.set('');try{
  const [materials,groups,prices,profiles,costs,orders,parts,pans]=await Promise.all([
 ...
 async saveCatalogProfile(part:any,lines:any[],work:any,pans:number|null,confirmed:boolean,expected:string|null){return this.write('wc_save_catalog_cost_profile',{p_main:part.main_item_id,p_item:part.item_id,p_key:part.variant_key,p_lines:lines.map(l=>({material_id:l.material_id,quantity:Number(l.quantity)})),p_work:work,p_pans:pans,p_confirmed:confirmed,p_expected:expected});}
 async saveCartMaterialProfile(part:any,lines:any[],confirmed:boolean,expected:string|null){return this.write('wc_save_cart_material_profile',{p_product:part.shipping_product_id,p_size:part.size_key,p_kind:part.kind,p_lines:lines.map(l=>({material_id:l.material_id,quantity:Number(l.quantity)})),p_confirmed:confirmed,p_expected:expected});}
 async saveBackdropMaterialProfile(part:any,lines:any[],confirmed:boolean,expected:string|null){return this.write('wc_save_shared_backdrop_material_profile',{p_product:part.shipping_product_id,p_folding:part.folding,p_lines:lines.map(l=>({material_id:l.material_id,quantity:Number(l.quantity)})),p_confirmed:confirmed,p_expected:expected});}
