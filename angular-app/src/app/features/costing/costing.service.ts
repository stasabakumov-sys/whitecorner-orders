...
export class CostingService {
 materials=signal<any[]>([]);groups=signal<any[]>([]);prices=signal<any[]>([]);profiles=signal<any[]>([]);costs=signal<any[]>([]);
 ...
 async load(){if(this.loading())return;this.loading.set(true);this.error.set('');try{
  const [materials,groups,prices,profiles,costs]=await Promise.all([
  ...
  this.materials.set(materials);this.groups.set(groups);this.prices.set(prices);this.profiles.set(profiles);this.costs.set(costs);
