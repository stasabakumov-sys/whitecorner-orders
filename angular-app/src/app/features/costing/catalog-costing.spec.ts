...
describe('Shared catalogue costing',()=>{
 ...
 it('saves a missing backdrop folding variant through its structural material profile',async()=>{const saveBackdropMaterialProfile=vi.fn().mockResolvedValue(true);const service:any={materials:signal([]),profiles:signal([]),saveBackdropMaterialProfile};const e=new CatalogCostEditorComponent(service);e.showWork=false;e.part={variant_key:'draft',backdrop_material_scope:true,shipping_product_id:'product',size_key:'2000x1000',folding:'nonfoldable'};e.lines=[];e.confirmed=true;await e.save();expect(saveBackdropMaterialProfile).toHaveBeenCalledWith(e.part,[],true,null);});
 it('saves a Cart size profile without requiring an order item',async()=>{const saveCartMaterialProfile=vi.fn().mockResolvedValue(true);const service:any={materials:signal([]),profiles:signal([]),saveCartMaterialProfile};const e=new CatalogCostEditorComponent(service);e.showWork=false;e.part={variant_key:'cart',cart_material_scope:true,shipping_product_id:'product',size_key:'size ii',kind:'main'};e.lines=[];e.confirmed=true;await e.save();expect(saveCartMaterialProfile).toHaveBeenCalledWith(e.part,[],true,null);});
});
