...
describe('Production comparison',()=>{
 ...
 it('shows a confirmed legacy cost while asking to confirm the shared profile',()=>{const legacy={...profile('NO',3),profile:null,legacy_profile:profile('NO',3).profile};const rows=calculate([template('nonfoldable',120)],[legacy],[false]);expect(rows[1].materials).toBe(30);expect(rows[1].total).toBe(150);expect(rows[1].issues).toContain('Confirm the shared materials profile');});
});
