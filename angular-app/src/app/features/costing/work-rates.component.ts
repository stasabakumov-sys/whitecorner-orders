...
export class WorkRatesComponent implements OnInit{
 rows:WorkRate[]=emptyWorkRates();draft:Record<string,number|null>=Object.fromEntries(this.rows.map(row=>[row.work_type,null]));loading=signal(false);busy=signal(false);error=signal('');done=signal(false);
 constructor(private db:SupabaseService){}ngOnInit(){void this.load();}
 async load(){
  this.loading.set(true);this.error.set('');this.done.set(false);
  const controller=new AbortController();
  let timer:ReturnType<typeof setTimeout>|undefined;
  try{
   const query=this.db.client.from('wc_work_rates').select('work_type,label,rate_gst_hour,sort_order,updated_at').order('sort_order').abortSignal(controller.signal);
   const {data,error}=await Promise.race([query,new Promise<never>((_,reject)=>{timer=setTimeout(()=>{reject(new Error('timeout'));controller.abort();},10000);})]);
   if(error)throw error;
   const saved=(data||[]) as WorkRate[];
   this.rows=emptyWorkRates().map(row=>saved.find(item=>item.work_type===row.work_type)||row);
   this.draft=Object.fromEntries(this.rows.map(row=>[row.work_type,row.rate_gst_hour]));
  }catch(error){this.error.set((error as Error)?.message==='timeout'?'Work rates are taking too long to load. Check the connection and Refresh.':'Could not load work rates. Refresh and retry.');}
  finally{if(timer)clearTimeout(timer);this.loading.set(false);}
 }
 ...
 async save(row:WorkRate){if(this.busy()||!this.valid(row))return;this.busy.set(true);this.error.set('');this.done.set(false);try{const {error}=await this.db.client.rpc('wc_save_work_rate',{p_type:row.work_type,p_rate:Number(this.draft[row.work_type]),p_expected:row.updated_at});if(error){this.error.set(error.message||'Could not save work rate. Refresh and retry.');return;}await this.load();if(!this.error())this.done.set(true);}catch(e){this.error.set((e as Error)?.message||'Could not save work rate. Refresh and retry.');}finally{this.busy.set(false);}}
}
