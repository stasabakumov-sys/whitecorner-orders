...
describe('Shared Backdrop painting profile',()=>{
 ...
 it('saves painting minutes while preserving paint materials',async()=>{
  ...
  expect(rpc).toHaveBeenCalledWith('wc_save_backdrop_paint_profile',{p_product:'backdrop',p_lines:[{material_id:'paint',quantity:2}],p_estimates:{'Painting:Finish coat':15},p_confirmed:true,p_expected:1});expect(component.message).toContain('minutes');
 });
