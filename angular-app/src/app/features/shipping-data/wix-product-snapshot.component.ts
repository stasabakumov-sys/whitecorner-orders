...
@Component({selector:'app-wix-product-snapshot',standalone:true,template:`
 ...
 @if(product();as p){<section><h3>Wix catalogue · read only</h3>
 ...
 `,styles:[`section{border-top:1px solid #e7edf5;padding-top:12px}h3{font-size:15px}.scroll{max-height:280px;overflow:auto}table{width:100%;border-collapse:collapse;font-size:13px}td,th{text-align:left;padding:8px;border-bottom:1px solid #e7edf5}`]})
export class WixProductSnapshotComponent implements OnChanges{
 @Input({required:true})productId='';readonly product=signal<any>(null);readonly error=signal('');private request=0;
 constructor(private readonly supabase:SupabaseService){}
 choices(value:any){return value&&typeof value==='object'?Object.entries(value).map(([k,v])=>`${k}: ${String(v)}`).join(' · '):'Default';}
 async ngOnChanges(){const request=++this.request;this.product.set(null);this.error.set('');if(!this.productId||this.productId.startsWith('saved:'))return;
