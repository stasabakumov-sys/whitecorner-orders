...
describe('Cart Main packaging variants',()=>{
 ...
 it('puts Internal Shelf inside the manually entered Main composition',()=>{
  const{component}=setup();component.shelf='yes';
  expect(component.components().map(c=>c.component_key).sort()).toEqual(['main','option:internal shelf']);
  component.shelf='no';expect(component.components().map(c=>c.component_key)).toEqual(['main']);
 });
 it('saves the complete dimensions as a Cart Main profile without calculating them',async()=>{
  const{component,invoke}=setup();component.shelf='yes';component.boxes=[{package_name:'Top/Bottom + shelf',length_mm:1430,width_mm:630,height_mm:110,weight_kg:22.5,contents:component.components()}];component.confirmed=true;
