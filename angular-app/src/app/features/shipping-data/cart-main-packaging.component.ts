...
@Component({selector:'app-cart-main-packaging',standalone:true,imports:[FormsModule],template:`
 <section class="editor">
  <h4>Main + Internal Shelf variant</h4>
  <p class="mut">The reusable Main above remains unchanged. Enter the complete replacement packaging used only when Internal Shelf is selected.</p>
  <div class="states"><span [class.ready]="hasVariant()">{{hasVariant()?'Variant saved ✓':'Variant required before quoting'}}</span></div>
  @if(error()){<p role="alert">{{error()}}</p>}
  <h5>{{sizeLabel}} · Main with Internal Shelf</h5>
  ...
  <button type="button" class="primary" [disabled]="busy()||!!issue()||!confirmed" (click)="save()">{{busy()?'Saving…':'Save Main + Shelf variant'}}</button>
  @if(saved()){<p role="status">Main + Shelf variant saved ✓</p>}
 </section>
...
export class CartMainPackagingComponent implements OnChanges{
 ...
 async load(){if(!this.product?.id||!this.sizeKey)return;this.busy.set(true);this.error.set('');try{const [profiles,rules]=await Promise.all([this.db.client.from('wc_delivery_packaging_profiles').select('*').eq('shipping_product_id',this.product.id),this.db.client.from('wc_shipping_rules').select('match_name,match_value,effect_type,active').eq('active',true).eq('effect_type','No effect')]);if(profiles.error||rules.error)throw Error('Main + Shelf variant could not be loaded.');this.variants.set((profiles.data||[]).filter((p:any)=>p.template_item?.profile_scope==='cart-main'&&cartSizeFromOptions(p.template_item?.wix_options)===this.sizeKey&&this.variantHasShelf(p)));this.rules=rules.data||[];const profile=this.variants()[0];this.boxes=profile?structuredClone(profile.packages||[]):[];if(profile)this.remap();}catch(e:any){this.error.set(e.message||'Main + Shelf variant could not be loaded.');}finally{this.busy.set(false);}}
 item(){return{id:this.product.id,product_name:this.product.product_name,quantity:1,catalog_reference:this.product.wix_product_id?{catalogItemId:this.product.wix_product_id}:{},wix_options:{Size:this.sizeLabel,'Internal Shelf':'Yes'}};}
 components(){return reviewComponents({wc_order_items:[this.item()]},this.rules);}
 variantHasShelf(profile:any){return String(profile.template_item?.wix_options?.['Internal Shelf']||'').toLowerCase()==='yes';}
 ...
 async save(){if(this.busy()||this.issue()||!this.confirmed)return;this.busy.set(true);this.error.set('');this.saved.set(false);try{const {data,error}=await this.db.client.functions.invoke('delivery-cost-review',{body:{action:'save-packaging-variant',profileScope:'cart-main',productId:this.product.id,options:[{name:'Size',value:this.sizeLabel},{name:'Internal Shelf',value:'Yes'}],packages:this.boxes}});if(error||!data?.ok){const detail=await error?.context?.json?.().catch(()=>null);throw Error(detail?.error||data?.error||'Main + Shelf variant was not saved.');}await this.load();this.confirmed=false;this.saved.set(true);}catch(e:any){this.error.set((e?.message||'Main + Shelf variant was not saved.')+' Your entries are retained; retry after checking the connection.');}finally{this.busy.set(false);}}
}
