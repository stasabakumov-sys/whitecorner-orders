...
import {paintOperations,paintLabel} from '../shop-floor/shop-floor.models';

@Component({selector:'app-backdrop-paint-profile',standalone:true,imports:[CommonModule,FormsModule],template:`
<details class="paint" [open]="mode==='minutes'"><summary>{{mode==='materials'?'Paint materials':'Painting minutes'}} · shared by all variants</summary>
 @if(mode==='materials'){
 <p>Enter paint materials once for this product. They are used only when the ordered finish is Painted.</p>
 <fieldset [disabled]="busy"><legend>Paint materials for one product</legend>
  ...
  <button class="primary" type="button" [disabled]="invalid()" (click)="save()">{{busy?'Saving…':'Save paint materials'}}</button>
 </fieldset>
 }@else{
 <p>Enter painting time once. RAW orders exclude it; Painted orders add it to the structural template.</p>
 <fieldset [disabled]="busy"><legend>Painting minutes for one product</legend>
  ...
  <button class="primary" type="button" [disabled]="invalid()" (click)="save()">{{busy?'Saving…':'Save painting minutes'}}</button>
 </fieldset>
 }
...
export class BackdropPaintProfileComponent implements OnChanges{
 ...
 async save(){if(this.busy||this.invalid())return;this.busy=true;this.message='';this.error='';try{const {data,error}=await this.db.client.rpc('wc_save_product_paint_profile',{p_product:this.product.id,p_lines:this.lines.map(line=>({material_id:line.material_id,quantity:Number(line.quantity)})),p_estimates:this.estimates,p_confirmed:this.confirmed,p_expected:this.version});if(error)throw error;if(!data||Number(data.version)!==this.version+1)throw Error('The server did not confirm the painting profile.');this.version=Number(data.version);this.message=this.mode==='materials'?'Paint materials saved.':'Painting minutes saved.';this.saved.emit({...this.product,backdrop_paint_profile:data});}catch(e:any){this.error=e?.message||'Could not save shared painting. Retry.';}finally{this.busy=false;}}
}
