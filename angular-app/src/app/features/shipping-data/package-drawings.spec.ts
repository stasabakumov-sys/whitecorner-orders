...
describe('One shared packing drawing',()=>{
 ...
 it('provides one upload only when neither qualified nor unclassified drawing exists',async()=>{
  const {f}=await render(true,'2000x1000:nonfoldable');expect(f.nativeElement.querySelectorAll('app-box-drawing input[type=file]')).toHaveLength(1);expect(f.nativeElement.textContent).toContain('Upload drawing');
 });
 it('shows the unclassified existing drawing for review and blocks a second upload',async()=>{
  ...
  expect(f.nativeElement.textContent).toContain('legacy.cdr');expect(f.nativeElement.textContent).toContain('classify');expect(f.nativeElement.querySelectorAll('app-box-drawing input[type=file]')).toHaveLength(0);
 });
