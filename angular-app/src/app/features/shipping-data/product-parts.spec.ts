...
describe('Product-owned Shop Floor parts',()=>{
 ...
 it('opens only the ordered construction in Board',async()=>{const {component,templates}=setup();component.product={id:'main',product_type:'Backdrop'};component.orderVariant=true;component.selectedFolding='nonfoldable';templates.push({id:'folded',name:'Folded',folding:'foldable',size_key:null,parts:[],estimates:{CNC:10},version:1},{id:'ordered',name:'Flat',folding:'nonfoldable',size_key:null,parts:[],estimates:{CNC:20},version:1});await component.load();expect(component.templates.map(row=>row.id)).toEqual(['ordered']);expect(component.editingId).toBe('ordered');});
 it('hides unordered add-on parts in Board and preserves them when saving',async()=>{const {component,templates,rpc}=setup();component.product={id:'main',product_name:'Display Unit'};component.orderVariant=true;component.orderComponentIds=['main'];templates.push({id:'template',name:'Display Unit',product_id:'main',size_key:null,folding:null,version:1,parts:[{id:'body',name:'Body',component_product_id:'main'},{id:'addon-part',name:'Back panel',component_product_id:'addon'}],estimates:{CNC:10,'Assembly:body':20,'Assembly:addon-part':60}});await component.load();expect(component.parts.map(part=>part.id)).toEqual(['body']);expect(component.estimates['Assembly:addon-part']).toBeUndefined();await component.save();expect(rpc).toHaveBeenLastCalledWith('wc_shop_save_product_template',expect.objectContaining({p_parts:[expect.objectContaining({id:'body'}),expect.objectContaining({id:'addon-part'})],p_estimates:expect.objectContaining({'Assembly:addon-part':60})}));});
