...
describe('Product-owned Shop Floor parts',()=>{
 ...
 it('retains independent folding drafts and saves the selected structural variant',async()=>{const {component,rpc}=setup();component.product={id:'main',product_name:'Arch Backdrop'};component.sizes=['200cm x 100cm'];await component.load();component.chooseVariant('foldable');component.addPart();component.parts[0].name='Folded body';component.estimates={CNC:10};component.chooseVariant('nonfoldable');expect(component.parts).toEqual([]);component.addPart();component.parts[0].name='Flat body';component.estimates={CNC:20};component.chooseVariant('foldable');expect(component.parts[0].name).toBe('Folded body');expect(component.estimates['CNC']).toBe(10);rpc.mockImplementation(async(name,args)=>({data:{id:'folded',product_id:args.p_product,name:args.p_name,parts:args.p_parts,estimates:args.p_estimates,version:1,size_key:args.p_size,folding:args.p_folding},error:null}));await component.save();expect(rpc).toHaveBeenCalledWith('wc_shop_save_variant_template',expect.objectContaining({p_size:'2000x1000',p_folding:'foldable',p_estimates:{CNC:10}}));expect(component.done).toBe(true);component.chooseVariant('nonfoldable');expect(component.estimates['CNC']).toBe(20);});
 ...
 it('keeps the chosen size and folding option when an unassigned template is opened',async()=>{const {component,rpc,templates}=setup();component.product={id:'main',product_name:'Arch Backdrop'};component.sizes=['200cm x 100cm','180cm x 90cm'];templates.push({id:'legacy',product_id:'main',name:'Plane Arch',parts:[{id:'body',name:'Backdrop body',component_product_id:'main'}],estimates:{CNC:10},version:2,size_key:null,folding:null});await component.load();component.chooseSize('2000x1000');component.chooseVariant('foldable');component.editTemplate(templates[0]);expect(component.sizeKey).toBe('2000x1000');expect(component.folding).toBe('foldable');rpc.mockImplementation(async(name,args)=>({data:{...templates[0],size_key:args.p_size,folding:args.p_folding,version:3},error:null}));await component.save();expect(rpc).toHaveBeenCalledWith('wc_shop_save_variant_template',expect.objectContaining({p_id:'legacy',p_size:'2000x1000',p_folding:'foldable'}));expect(component.done).toBe(true);});
});
