...
@Component({selector:'app-saved-packing',standalone:true,imports:[FormsModule,PackageDrawingsComponent],template:`
 ...
 @if(editing){
 <td><input aria-label="Box name" [disabled]="saving()||backdrop" [(ngModel)]="box.package_name"></td>
 <td><input aria-label="Length mm" type="number" min="1" [disabled]="saving()||backdrop" [(ngModel)]="box.length_mm"></td>
 <td><input aria-label="Width mm" type="number" min="1" [disabled]="saving()||backdrop" [(ngModel)]="box.width_mm"></td>
 <td><input aria-label="Height mm" type="number" min="1" [disabled]="saving()||backdrop" [(ngModel)]="box.height_mm"></td>
 ...
 <td>{{canonical(box).package_name}}</td><td>{{canonical(box).length_mm}}</td><td>{{canonical(box).width_mm}}</td><td>{{canonical(box).height_mm}}</td><td>{{box.weight_kg}}</td>
...
export class SavedPackingComponent{
 ...
 async save(){
  ...
  try{
   ...
   const {data,error}=await this.db.client.functions.invoke('delivery-cost-review',{body:{action:'save-packaging-variant',productId:this.product.id,sourceItemId:this.profile.template_item?.source_item_id||'',existingSignature:this.profile.signature,options,packages}});
   ...
   this.profileSaved.emit({...this.profile,signature:data.signature||this.profile.signature,packages});this.editing=false;this.replacing=false;this.saved.set(true);
