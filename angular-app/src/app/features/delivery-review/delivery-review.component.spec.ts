...
describe('Delivery review UI',()=>{
 ...
 it('shows excluded carriers in the persisted quote, displays the shortfall and requires an exception reason',async()=>{
  ...
  const row={order_id:'order',state:'quoted',wc_orders:order,input_key:reviewInputKey(order),quote_attempted_at:new Date().toISOString(),packages:[],evaluated_quotes:evaluateQuotes([{courierName:'Unapproved carrier',priceIncludingGst:1},{courierName:'Aramex',priceIncludingGst:95}],insuranceFor(['Free up to $500'],11000))};
  ...
  const body=f.nativeElement.textContent;expect(body).toContain('Unapproved carrier');expect(body).toContain('Carrier excluded by policy');expect(body).toContain('$5.56');expect(body).toContain('Price review required');
