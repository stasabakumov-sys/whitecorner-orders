...
@Component({
  ...
  template:`
    <p-drawer [visible]="true" position="right" [modal]="true" [dismissible]="true" [style]="{width:'min(1160px,90vw)'}" (onHide)="closed.emit()">
      ...
      @if(!order.is_history){<div class="activity-full"><app-order-activity [order]="order" /></div>}
  ...
  styles: [`@layer hub-layout {
    ...
    .page-grid{display:grid;grid-template-columns:minmax(0,1fr) 310px;gap:18px;padding:14px 4px 0}.main-column,.side-column{min-width:0}.activity-full{padding:0 4px 24px}.wix-section{background:#fff;border:1px solid #e4e7ec;border-radius:12px;overflow:hidden;margin-bottom:18px}.wix-section-head{min-height:56px;padding:0 22px;display:flex;align-items:center;gap:10px;border-bottom:1px solid #e4e7ec;font-size:17px;color:#101828}.items-group-label{padding:10px 22px;background:#edf4ff;border-bottom:1px solid #d7e5ff;font-weight:500;color:#344054}.addons-label{border-top:1px solid #e4e7ec;background:#f8fafc}
