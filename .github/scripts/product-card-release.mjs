...
const names=['20260913000100_backdrop_drawing_folding','20260913000200_backdrop_paint_operations','20260916000200_backdrop_drawing_links','20260918000100_backdrop_packaging_dimensions'];
...
const verify=`select
 (select count(*)::int from supabase_migrations.schema_migrations where version in ('20260913000100','20260913000200','20260916000200','20260918000100')) registered,
 ...
 (select count(*)::int from pg_class where oid in (to_regclass('public.wc_shop_units'),to_regclass('public.wc_backdrop_box_drawings'),to_regclass('public.wc_backdrop_packaging_dimensions')) and relrowsecurity) rls_tables,
...
do $verify$ begin
 if has_function_privilege('anon','public.wc_classify_backdrop_box_drawing(text,text,uuid)','EXECUTE') or has_function_privilege('anon','public.wc_save_backdrop_packaging_dimensions(text,text,numeric,numeric,numeric,uuid)','EXECUTE') or has_function_privilege('anon','public.wc_shop_command(uuid,text,jsonb)','EXECUTE') then raise exception 'Anonymous write access';end if;
 if not has_function_privilege('authenticated','public.wc_classify_backdrop_box_drawing(text,text,uuid)','EXECUTE') or not has_function_privilege('authenticated','public.wc_save_backdrop_packaging_dimensions(text,text,numeric,numeric,numeric,uuid)','EXECUTE') then raise exception 'Backdrop reference contracts unavailable';end if;
 if (select count(*) from pg_class where oid in ('public.wc_shop_units'::regclass,'public.wc_backdrop_box_drawings'::regclass,'public.wc_backdrop_packaging_dimensions'::regclass) and relrowsecurity)<>3 then raise exception 'RLS missing';end if;
end $verify$;
commit;
${verify}`;
...
if(process.argv.includes('--apply')){await request(sql,false);const result=await request(verify,true);const r=result[0];if(r?.registered!==4||!r.paint_routes||!r.classification||!r.backdrop_dimensions_save||r.rls_tables!==3||r.anon_shop_write)throw Error('Production verification failed');console.log('Verified:',JSON.stringify(result));}
