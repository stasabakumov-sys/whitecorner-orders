...
const names=['20260913000100_backdrop_drawing_folding','20260913000200_backdrop_paint_operations','20260916000200_backdrop_drawing_links'];
...
const verify=`select
 (select count(*)::int from supabase_migrations.schema_migrations where version in ('20260913000100','20260913000200','20260916000200')) registered,
...
do $prerequisites$ begin
 ...
 if to_regprocedure('public.wc_shop_metric_size(text)') is null then raise exception 'Backdrop size parser prerequisite missing';end if;
...
if(process.argv.includes('--apply')){await request(sql,false);const result=await request(verify,true);const r=result[0];if(r?.registered!==3||!r.paint_routes||!r.classification||r.rls_tables!==2||r.anon_shop_write)throw Error('Production verification failed');console.log('Verified:',JSON.stringify(result));}
