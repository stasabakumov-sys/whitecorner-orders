...
try{
 ...
 create table wc_order_items(id uuid primary key,wix_options jsonb,custom_text_fields jsonb,catalog_reference jsonb not null default '{}',description_lines jsonb not null default '[]',raw_item jsonb not null default '{}');
 ...
 create function wc_shop_product_components(p_product uuid) returns table(id uuid,product_name text,component_role text) language sql stable as $$select p.id,p.product_name,'Product'::text from wc_shipping_products p where p.id=p_product$$;
 ...
 create function wc_catalog_text(v jsonb) returns text language plpgsql immutable as $$declare k text;result text;begin if jsonb_typeof(v) in('string','number','boolean') then return btrim(v#>>'{}');end if;if jsonb_typeof(v)='object' then foreach k in array array['original','translated','value','name','description','text','plainText','plainTextValue','label','title'] loop result=wc_catalog_text(v->k);if nullif(result,'') is not null then return result;end if;end loop;end if;return '';end$$;
 ...
 create function wc_shop_item_product(uuid) returns uuid language sql stable as $$select id from wc_shipping_products where product_type='Backdrop' limit 1$$;`);
 ...
 const tableProduct=randomUUID();await db.query("insert into wc_shipping_products(id,product_name,product_type) values($1,'Portable Market Table','Table')",[tableProduct]);await db.query("insert into wc_shop_templates(product_id,name,parts,estimates) values($1::uuid,'Table',jsonb_build_array(jsonb_build_object('id','table','name','Table','component_product_id',$1::text)),jsonb_build_object('CNC',10,'Painting:First primer',20))",[tableProduct]);
 const sharedRelease=execFileSync(process.execPath,['.github/scripts/shared-painting-release.mjs','--print-sql'],{encoding:'utf8'});await db.exec(sharedRelease);await db.exec(sharedRelease);
 assert.equal((await db.query("select count(*)::int n from supabase_migrations.schema_migrations where version='20260917000200'")).rows[0].n,1);
 const migrated=(await db.query('select backdrop_paint_profile from wc_shipping_products where id=$1',[tableProduct])).rows[0].backdrop_paint_profile;assert.equal(migrated.estimates['Painting:First primer'],20);assert.equal((await db.query("select estimates ? 'Painting:First primer' has_paint from wc_shop_templates where product_id=$1",[tableProduct])).rows[0].has_paint,false);
 const structural=(await db.query("select wc_shop_save_product_template(null,$1,'New table',$2,$3,null) value",[tableProduct,[{id:'table',name:'Table',component_product_id:tableProduct}],{CNC:12,'Painting:Finish coat':99}])).rows[0].value;assert.equal(structural.estimates.CNC,12);assert.equal(structural.estimates['Painting:Finish coat'],undefined);
 const shared=(await db.query('select wc_save_product_paint_profile($1,$2,$3,true,$4) value',[tableProduct,[{material_id:material,quantity:1}],{'Painting:Finish coat':30},migrated.version])).rows[0].value;assert.equal(shared.estimates['Painting:Finish coat'],30);
 const foldingRelease=execFileSync(process.execPath,['.github/scripts/shop-floor-order-folding-release.mjs','--print-sql'],{encoding:'utf8'});await db.exec(foldingRelease);await db.exec(foldingRelease);
 assert.equal((await db.query("select count(*)::int n from supabase_migrations.schema_migrations where version='20260923000200'")).rows[0].n,1);
 const fallbackItem=randomUUID();await db.query('insert into wc_order_items(id,wix_options,description_lines) values($1,$2,$3)',[fallbackItem,{},[{name:{original:'Foldable'},plainText:{original:'YES'}}]]);
 assert.equal((await db.query('select wc_shop_item_folding(i) folding from wc_order_items i where id=$1',[fallbackItem])).rows[0].folding,'foldable');
 const rawDescriptionItem=randomUUID();await db.query('insert into wc_order_items(id,wix_options,raw_item) values($1,$2,$3)',[rawDescriptionItem,{}, {descriptionLines:[{name:{original:'Foldable'},plainText:{original:'YES'}}]}]);
 assert.equal((await db.query('select wc_shop_item_folding(i) folding from wc_order_items i where id=$1',[rawDescriptionItem])).rows[0].folding,'foldable');
 const fallbackUnit=randomUUID();await db.query('insert into wc_production_units values($1,$2)',[fallbackUnit,fallbackItem]);
 await db.query('select wc_shop_auto_snapshot($1)',[fallbackUnit]);
 assert.equal((await db.query('select template_id from wc_shop_units where unit_id=$1',[fallbackUnit])).rows[0].template_id,saved.id);
 await db.query('update wc_order_items set wix_options=$2 where id=$1',[fallbackItem,{Foldable:'NO'}]);
 assert.equal((await db.query('select wc_shop_item_folding(i) folding from wc_order_items i where id=$1',[fallbackItem])).rows[0].folding,null);
 const conflictingUnit=randomUUID();await db.query('insert into wc_production_units values($1,$2)',[conflictingUnit,fallbackItem]);
 await assert.rejects(db.query('select wc_shop_auto_snapshot($1)',[conflictingUnit]),/Foldable choice is missing or conflicting/);
 console.log('Shared Painting: Backdrops and other products keep structural templates separate from one optional Painting profile.');
