// Targeted, repeatable CRV3D release. Never runs db push.
import {readFile} from 'node:fs/promises';

const version='20260922000400',name='product_cnc_crv3d_file';
...
const verify=`select
 ...
 coalesce(position('.crv3d' in pg_get_functiondef(to_regprocedure('${signature}')))>0,false) crv3d_validator,
...
const sql=`begin;select pg_advisory_xact_lock(${version});do $release$ begin
 ...
 if not exists(select 1 from supabase_migrations.schema_migrations where version='20260922000300') then raise exception 'CNC CRC3D migration not registered';end if;
 if exists(select 1 from supabase_migrations.schema_migrations where version='${version}') then
  if (select replace(statements[1],E'\\r','') from supabase_migrations.schema_migrations where version='${version}') is distinct from replace(${literal},E'\\r','') then raise exception 'CRV3D migration differs from registered version';end if;
...
const before=(await query("select to_regclass('public.wc_product_cnc_sheets') is not null sheets_ready,exists(select 1 from supabase_migrations.schema_migrations where version='20260922000300') crc3d_ready;",true))[0];
console.log('Preflight:',JSON.stringify(before));if(!before?.sheets_ready||!before?.crc3d_ready)throw Error('Production prerequisites missing');
if(process.argv.includes('--verify')){console.log('Read-only CRV3D state:',JSON.stringify((await query(verify,true))[0]));process.exit(0);}
if(process.argv.includes('--apply')){await query(sql,false);const after=(await query(verify,true))[0];if(!after?.registered||!after?.attach_rpc||!after?.crv3d_validator||!after?.authenticated_execute||!after?.private_bucket)throw Error(`CRV3D migration postflight failed: ${JSON.stringify(after)}`);console.log('Verified:',JSON.stringify(after));}
