...
const verify=`select
 ...
 to_regprocedure('public.wc_shop_order_finish(jsonb)') is not null base_finish_resolver,
 to_regprocedure('public.wc_shop_resolved_variant_size(jsonb,text)') is not null resolved_size,
 to_regprocedure('public.wc_cart_size_key(jsonb)') is not null cart_size,
...
const sql=`begin;select pg_advisory_xact_lock(${version});do $release$ begin
 ...
 if to_regprocedure('public.wc_shop_order_finish(jsonb)') is null then raise exception 'Base finish resolver missing';end if;
 if to_regprocedure('public.wc_shop_resolved_variant_size(jsonb,text)') is null or to_regprocedure('public.wc_cart_size_key(jsonb)') is null then raise exception 'Product size resolver missing';end if;
...
if(!before?.auto_snapshot||!before?.base_finish_resolver||!before?.resolved_size||!before?.cart_size)throw Error('Production prerequisites missing');
