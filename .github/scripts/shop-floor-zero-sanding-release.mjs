...
const version='20260914000400',name='shop_floor_skip_zero_sanding';
...
const verify=`select
 ...
 to_regprocedure('public.wc_shop_status_guard()') is not null guard,
 coalesce((select pg_get_functiondef(p.oid) like '%sanding_required%' from pg_proc p where p.oid=to_regprocedure('public.wc_shop_status_guard()')),false) zero_sanding_guard;`;
const sql=`begin;select pg_advisory_xact_lock(${version});do $release$ begin
 ...
 if not coalesce((select pg_get_functiondef(p.oid) like '%sanding_required%' from pg_proc p where p.oid=to_regprocedure('public.wc_shop_status_guard()')),false) then raise exception 'Zero-Sanding guard unavailable';end if;
...
if(!before?.shop_floor_registered||!before?.auto_assignment_registered||!before?.guard)throw Error('Production prerequisites missing');
if(process.argv.includes('--apply')){await query(sql,false);const rows=await query(verify,true);if(!rows[0]?.registered||!rows[0]?.zero_sanding_guard)throw Error('Postflight failed');console.log('Verified:',JSON.stringify(rows));}
