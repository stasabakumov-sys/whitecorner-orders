...
try{
 ...
 const product=(await db.query("select id,product_name from wc_shipping_products where product_name like 'MDF Mobile%'" )).rows[0],mdf=product.id;
 const content={component_key:'main',product_name:product.product_name,profile_item_key:'["cart",["internal shelf no","side shelves no"]]:0'};
 await db.query("insert into wc_delivery_packaging_profiles(signature,shipping_product_id,template_item,packages) values('base',null,$1,$2)",[{wix_options:{'Internal Shelf':'No','Side shelves':'No'}},[{package_name:'Front',length_mm:1180,width_mm:670,height_mm:60,weight_kg:22.5,contents:[content]},{package_name:'Top',length_mm:1230,width_mm:630,height_mm:150,weight_kg:25,contents:[content]},{package_name:'Castors',length_mm:280,width_mm:150,height_mm:150,weight_kg:3,contents:[content]}]]);
 await db.exec("insert into wc_shipping_products(product_name,product_type) values('Another Mobile Cart','Other')");
 const migration=await readFile('supabase/migrations/20260914000600_cart_modular_packaging.sql','utf8'),backfill=await readFile('supabase/migrations/20260915000100_cart_modular_packaging.sql','utf8');await db.exec(migration);await db.exec(backfill);await db.exec(migration);await db.exec(backfill);
 ...
 assert.equal((await db.query("select shipping_product_id from wc_delivery_packaging_profiles where signature='base'")).rows[0].shipping_product_id,mdf);
