...
try {
 ...
 await db.exec(`create table wc_shipping_products(id uuid primary key default gen_random_uuid(),wix_product_id text unique,product_name text not null,product_type text default 'Other',active boolean default true);
 ...
 for(const name of ['20260907000800_material_costing','20260908000300_tabletop_material_costing','20260908000400_catalog_product_costing','20260912000200_hub_test_orders'])
 ...
 for(const table of ['wc_shipping_products','wc_product_costs','wc_order_pans_costs'])assert.equal((await db.query(`select count(*)::int n from ${table}`)).rows[0].n,0,table);
 ...
 console.log('PASS: two idempotent Hub fixtures, no external identity, delivery blocked, no catalogue/cost pollution, progress retained, real orders and RLS unchanged');
