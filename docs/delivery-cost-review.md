...
Initial estimates use complete packaging from Products automatically, including existing unquoted delivery orders. Delivery Cost Review is read-only for packaging: there are no box editors, duplicate profile saving, or packing confirmation checkboxes. Opening an unquoted order checks Products and runs the initial estimate; opening an already attempted order never repeats its quote. Actual courier booking remains a separate Fulfilment flow.

## Data and workflow

- Migration `20260907000100_delivery_cost_review.sql` creates `wc_delivery_reviews`, `wc_delivery_packaging_profiles`, `wc_delivery_booking_exemptions`, an INSERT-only order trigger and service-only claim/package/exception RPCs. It does not alter existing order rows, create bookings, call APIs or apply Email AI. It snapshots existing Ready/reached-Fulfilment orders as booking exemptions and seeds existing non-Ready delivery orders as `legacy_packaging_required` (now included in the automatic queue).
...
- Products is the source for reusable Main boxes, option/add-on rules and product-owned saved variants. Old order-only snapshots do not override current Products measurements. Unknown or incomplete composition stays blocked, with a link to Products. Required-component mapping is shared with Fulfilment.
- Complete packaging saved in Products is used on the next background worker pass or when opening an unquoted order. No additional packaging approval is needed in this report. Components may span boxes; invalid dimensions, duplicates and unassigned components still block calculation. Changes after a quote require the explicit Recalculate from Products action, which archives the prior attempt.
...
- The complete HTTP response/body is saved BEFORE interpreting it. All returned carriers are retained and visible. The browser displays the persisted result; all preparation and calculation happen on the server. Carrier eligibility is separate from storage.
- Aramex, Couriers Please, FedEx and TNT exact normalized aliases are considered. TNT retains its provider name. Previously excluded TNT results with valid saved prices and insurance are included under the current policy without requesting a new quote. Unknown carriers remain visible but excluded.
